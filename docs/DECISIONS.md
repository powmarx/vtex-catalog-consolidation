...
**The supplied data is inverted relative to production, which makes this the wrong thing to under-weight.** Measured: all 975 catalog names are English, with no distinctively Portuguese word and no accented character anywhere. Of 269 input records, 266 are English-named. The three exceptions are all Portuguese renderings of products the catalog already holds — `Câmera`/`Camera` (#18), `Roteador`/`Router` (#21), `Processador`/`Processor` (#28) — and they are graded in difficulty on purpose: the first differs only by an accent and NFKD stripping resolves it, while the other two require knowing Portuguese and no normalization reaches them.

That ratio is an artifact of the exercise, not of the domain. A commerce platform serving retailers across dozens of countries sees catalogs in many languages as the ordinary case, and the same product named differently per locale is the norm rather than three planted outliers. Sellers in one market may also list in a language other than that market's default.

So on this file, leaving two duplicates is a 0.2% problem. On a genuinely multi-language catalog, cross-language matching stops being an edge case and becomes the dominant failure mode — at which point the resolutions named below are not improvements to consider later, they are requirements. The right answer scales; the lexical key does not.

This is the limitation I would raise first in review, because it is the one where the supplied data most understates the difficulty.
